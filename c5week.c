...
// 상품 정보를 파일에 저장하는 함수
void saveToFile(char names[][MAX_NAME_LEN], int inventory[], int sales[], int stock[]) {
    ...
    for (int i = 0; i < MAX_PRODUCTS; i++) {
        fprintf(file, "%s %d %d %d\n", names[i], inventory[i], sales[i], stock[i]);
    }
    fclose(file);
...
// 파일에서 상품 정보를 불러오는 함수
void loadFromFile(char names[][MAX_NAME_LEN], int inventory[], int sales[], int stock[]) {
    ...
    for (int i = 0; i < MAX_PRODUCTS; i++) {
        fscanf(file, "%s %d %d %d", names[i], &inventory[i], &sales[i], &stock[i]);
    }
    fclose(file);
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 5: { // 상품정보 저장
                saveToFile(names, inventory, sales, stock);
                break;
            }

            case 6: { // 상품정보 불러오기
                loadFromFile(names, inventory, sales, stock);
                break;
            }
